...
export default class AvlTable extends React.Component {
    ...
    addFilter(val = '') {
        ...
        if (!filter) {
            filter = searchString.includes(';') ? searchString.split(';').map(ss => StringEqualityFilter(searchKey, ss)) : StringEqualityFilter(searchKey, searchString);
        }
    ...
    render() {
        let [keys, originalData] = this.getKeysAndData();
        ...
        const {rowsPerPage, pageSpread} = this.props,
            maxPage = Math.max(Math.ceil(originalData.length / rowsPerPage) - 1, 0),
            length = originalData.length;

        page = Math.min(maxPage, page);

        let data = originalData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
        return (
            <DivContainer>
